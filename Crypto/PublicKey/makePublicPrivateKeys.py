...
def makeKeyFiles(name, path, keySize):
    ...
    if os.path.exists(f'{path}{name}_pubkey.txt') or os.path.exists(f'{path}{name}_privkey.txt'):
        sys.exit(
            f'WARNING: the file {path}{name}_pubkey.txt or {path}{name}_privkey.txt already exists! '
